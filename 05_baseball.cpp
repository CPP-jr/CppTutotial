...
int main()
{
	...
	while (true)
	{
		...
		for (int i = 0; i < 3; ++i)
		{
			...
			for (int j = 0; j < 3; ++j)
			{
				// i가 0일 때 j는 0 ~ 2 까지 반복하게 된다. 즉, 맞춰야할 숫자의
				// 첫번째 값과 입력받은 숫자의 첫번째, 두번째, 세번째를 차례대로 비교하여
				// 같은 값이 있는지를 찾아낸다.
				if (iNumber[i] == iInput[j])
				{
					// 값이 같은 것이 있을 경우 Strike 아니면 Ball이므로 자리가 같은
					// 지를 체크해본다. i와 j는 현재 맞춰야할 숫자의 인덱스과
					// 입력받은 숫자의 인덱스이고 두 값이 같으므로 이 인덱스가
					// 곧 위치가 된다. 인덱스가 서로 같은 경우라는 자리까지 같다는
					// 의미이므로 Strike를 증가하고 극 ㅔ아니라면 자리가 다르다는
					// 의미이므로 Ball을 증가하게 한다.
					if (i == j)
						++iStrike;
					else
