...
void CStage::Render()
{
	...
	이동한 위치로부터 맵을 출력해주어야 한다. 출력 크기는 세로 4칸
	가로 10캉으로 해준다. 지금은 플레이어가 처음에 있다 라고 가정하고
	처음 4 x 10 출력을 해준다.
	...
	*/
	...
	// 구해준다. 플레이어 위치보다 2칸 아래까지 출력해야하기 때문이다.
	...
	// 출력해야 할 최소 인덱스를 구해준다.
	// 출력해야할 가장 아래쪽 인덱스에서 출력해야할 개수 - 1개를 빼주게 되면
	...
	// 가로중 최대 출력수는 현재 플레이어 위치 + 출력해야할 가로개수이다.
	...
	for (int i = iYMin; i <= iYCount; ++i)
	{
		...
		{
			if ((i == iY || (i == iY - 1 && pPlayer->GetBigItemEnalbe()))
				&& j == iX)
				cout << "§";
...
bool CStage::ChechMonster(int x, int y)
{
	for (int i = 0; i < m_iMonsterCount; ++i)
	{
