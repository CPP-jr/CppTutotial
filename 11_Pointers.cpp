...
int main()
{
	...
	*/
	...
	단순히 메모리주소값이 1 증가하는 것이 아니다. int 포인터의 경우
	int 타입의 메모리 주소를 갖게 되는데 메모리 주소에 1을 더해주면
	1증가가 아닌 가리키는 해당 포인터타입의 변수타입 크기만큼 증가하게된다.
	...
	*/
	...
	// 문법이다. 그러므로 *pStudent를 괄고로 감싸준 수에 .을 이용해서
	...
	// 모든 저장가능하다. 단, 역참조가 불가능하고 메모리 주소만 저장
	// 가능하다.
	void*	pVoid = &iNumber;
