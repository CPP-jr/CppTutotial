...
// 숫자 입력을 받는다. 입력오류까지 처리해주고 INT_MAX를 리턴하면
// 입력오류이다.
int InputInt()
{
	int iInput;
...
bool LoadPlayer(_tagPlayer* pPlayer)
{
	...
	if (pFile)
	{
		...
		// 만약 저장할때 무기를 장착하고 있었다면 해당 무기정보도
		// 같이 저장이 되었다. 그러므로 여기서 장착하고 있을 경우
		// 읽어야한다.
		if (pPlayer->bEquip[EQ_WEAPON])
...
void SetMonster(_tagMonster* pMonsterArr)
{
	...
	pMonsterArr[2] = CreateMonster("드래곤", 250, 500, 200, 400,
		30000, 20000, 10, 3000, 20000, 50000);
	*/
}
...
int OutputInventory(_tagPlayer* pPalyer)
{
	system("cls");
	cout << "*************************** 가방 ***************************" << endl;

	OutputPlayer(pPalyer);

	for (int i = 0; i < pPalyer->tInventory.iItemCount; ++i)
	{
		cout << i + 1 << ". 이름 : " << pPalyer->tInventory.tItem[i].strName <<
			"\t종류 : " << pPalyer->tInventory.tItem[i].strTypeName << endl;
		cout << "공격력 : " << pPalyer->tInventory.tItem[i].iMin << " - " <<
			pPalyer->tInventory.tItem[i].iMax << endl;
		cout << "판매가격 : " << pPalyer->tInventory.tItem[i].iPrice <<
			"\t구매가격 : " << pPalyer->tInventory.tItem[i].iSell << endl;
		cout << "설명 : " << pPalyer->tInventory.tItem[i].strDesc << endl << endl;
	}

	cout << pPalyer->tInventory.iItemCount + 1 << ". 뒤로가기" << endl;
	cout << "장착할 아이템을 선택하세요 : ";
	int iMenu = InputInt();

	if (iMenu < 1 || iMenu > pPalyer->tInventory.iItemCount + 1)
		return INT_MAX;
...
int main()
{
	...
	tStoreArmor[0] = CreateItem("천옷", IT_ARMOR, 2, 5,
		1000, 500, "천으로 만든 옷");
